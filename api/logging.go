...
func (lm *loggingMiddleware) GetChainCA(ctx context.Context, session authn.Session, token string) (cert certs.Certificate, err error) {
	defer func(begin time.Time) {
	...
	return lm.svc.GetChainCA(ctx, session, token)
}
